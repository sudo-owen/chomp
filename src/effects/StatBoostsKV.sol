 ...
 *
 * Key difference from StatBoosts.sol:
 ...
 */
contract StatBoostsKV is BasicEffect {
    ...
    function _packSlotData(uint64 truncatedKey, bool isPerm, uint8[5] memory boostPercents, uint8[5] memory boostCounts, bool[5] memory isMultiply)
        internal
        pure
        returns (uint192)
    {
    ...
    function _unpackSlotData(uint192 data)
        internal
        pure
        returns (uint64 truncatedKey, bool isPerm, uint8[5] memory boostPercents, uint8[5] memory boostCounts, bool[5] memory isMultiply)
    {
    ...
    function _packBoostSnapshot(uint32[5] memory stats) internal pure returns (uint192) {
        return uint192(
            (uint256(stats[0]) << 160) | (uint256(stats[1]) << 128)
                | (uint256(stats[2]) << 96) | (uint256(stats[3]) << 64)
                | (uint256(stats[4]) << 32)
        );
    }
    ...
    function _addStatBoostsWithKey(
        ...
        (bool effectFound, uint256 effectIndex, bytes32 extraData) = _findEffectIndex(battleKey, targetIndex, monIndex);
        ...
        if (existingSlotIndex != 0) {
            ...
            (, bool existingIsPerm, uint8[5] memory existingPercents, uint8[5] memory existingCounts, bool[5] memory existingIsMultiply) =
            ...
            uint192 newSlotData = _packSlotData(key, existingIsPerm, existingPercents, existingCounts, existingIsMultiply);
    ...
    function _removeStatBoostsWithKey(uint256 targetIndex, uint256 monIndex, uint64 key, StatBoostFlag boostFlag) internal {
        bool isPerm = boostFlag == StatBoostFlag.Perm;
        bytes32 battleKey = ENGINE.battleKeyForWrite();

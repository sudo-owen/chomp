...
contract StatBoosts is BasicEffect {
    ...
    function onApply(uint256, bytes memory extraData, uint256 targetIndex, uint256 monIndex) external override returns (bytes memory, bool) {
        return ("", false);
    }

    function onMonSwitchOut(uint256, bytes memory extraData, uint256 targetIndex, uint256 monIndex) external override returns (bytes memory, bool) {
        return ("", false);
    }
    ...
    function _packBoostInstance(uint216 key, uint32 boostPercent, uint8 statType, bool isMultiply)
        internal
        pure
        returns (uint256)
    {
    ...
    function _unpackBoostInstance(uint256 packedBoostInstance)
        internal
        pure
        returns (uint216, uint32, uint8, bool)
    {
        ...
        return (key, boostPercent, statType, isMultiply);
    }

    // Public methods for adding/removing
